...
class XView2:
    ...
    def generate_dmg_segmaps(self, split=1.0):
        ...
        for j in tqdm(train_jsons):
            segmap = utils_post.generate_segmap(self.anndf, j)
    ...
    def generate_coco(self, split=1.0):
        ...
        if split == 1.0:
            # No split
            utils_post.generate_coco(self.predf, self.postdf, coco_dir)
        else:
            # Perform split -> train = split, val = (1 - split)
            utils_post.generate_coco_split(self.predf, self.postdf, split, coco_dir)
    ...
    def show_anns(self, ann):
        ...
        # Get polygons
        plist, dmg = utils_post.get_object_polygons(self.anndf, ann, _type="poly")
